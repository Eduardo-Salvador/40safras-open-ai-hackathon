...
describe("weather forecast normalization", () => {
  ...
  it("identifies a low-rain three-day window", () => {
    ...
    expect(signals).toContainEqual(expect.objectContaining({ code: "dry_window", severity: "favorable" }));
  });
  ...
  it("rejects invalid coordinates and payloads", async () => {
    ...
    const fetchMock = vi.fn(async () => new Response(JSON.stringify({ invalid: true }), { status: 200 }));
    await expect(fetchWeatherForecast(0, 0, fetchMock as typeof fetch)).rejects.toThrow("invalid forecast payload");
  });
});
