...
export function deriveWeatherSignals(days: ForecastDay[]): WeatherSignal[] {
  ...
  const maxThreeDayProbability = Math.max(
    0,
    ...nextThree.map((day) => day.precipitationProbabilityPct ?? 0),
  );
    ...
    : [
        {
          ...
          detail:
            "Os limiares determinísticos do protótipo não identificaram chuva intensa, calor ou vento fortes.",
        },
      ];
}
...
export async function fetchWeatherForecast(
  ...
  return {
    ...
    disclaimer:
      "Previsão de uma célula meteorológica próxima ao centroide, não uma medição dentro do talhão. Apoio operacional de protótipo; não substitui estação local, agrônomo ou ZARC.",
  };
}
